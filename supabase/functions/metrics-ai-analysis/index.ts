...
- Sé breve y directo (máx 350 palabras).`;
    ...
    if (aiRes.status === 402) {
      return new Response(JSON.stringify({ error: "Se requieren créditos en tu workspace de Lovable AI." }), { status: 402, headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }
    if (!aiRes.ok) {
